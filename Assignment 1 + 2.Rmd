...
### Results for Percent Households with Children and Race - Raveena
```{r}
Rav_cont_summary <- tibble(
  Variable = c("% Households with Children", "% White", "% Black", "% AI/AN", "% Asian", "% Hawaiian", "% Other", "% Two or More"),
...
kable(Rav_cont_summary, digits = 0)
...
grid.arrange(pretty_children_hist, pretty_RWhite_hist, pretty_RBlack_hist, pretty_RAI_hist, pretty_RAsian_hist, pretty_ROther_hist, pretty_RTwoPlus_hist,
             ncol = 3)
```
...
```
### Results for Poverty Rate - Alanna
```{r,  fig.height=3 and fig.width=7}
Alanna_cont_summary <- tibble(
  Variable = c("Poverty Rate"),
  `Sample mean` = c(pov_t_test$estimate),
  `Population mean (95% confidence) - low` = 
    c(pov_t_test$conf.int[1]),
  `Population mean (95% confidence) - high` =
    c(pov_t_test$conf.int[2]),
  Median = c(pov_quartiles[3]),
  `Interquartile range` = c(pov_quartiles[4] - pov_quartiles[2]),
  `Standard deviation` = c(pov_st_dev))

kable(Alanna_cont_summary, digits = 1)
