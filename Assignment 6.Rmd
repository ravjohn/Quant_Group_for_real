...
# Research Question
How do food access and demographic characteristics impact adult mental health on a census tract scale?
...
# Data
The sample population for this study is the full set of all census tracts in the United States for people over the age of 18. The analysis includes the following variables:
...
```
The correlation for mental health and median income is -0.7605683 and the 95% confidence interval is below zero. This indicates we can say with 95% that there is a negative correlation between mental health and median income. As the median income increases within a census tract, the percent of adults reporting poor mental health decreases. 
...
```
The correlation for mental health and poverty rate is 0.8124621 and the 95% confidence interval is above zero. This indicates we can say with 95% that there is a correlation between mental health and a census tract's poverty rate. As the percent of poverty increases, so does the percent of adults reporting poor mental health.  
...
*The USDA defines urban census tracts within 1 mile of food and rural census blocks within 10 miles of food as having food access.
...
interact_plot(interaction_SNAP_ED_model, pred = pct_SNAP, modx = MaxEd,
              interval = TRUE) +
  scale_x_continuous(name = "SNAP Utilization") +
  scale_y_continuous(name = "Percent of Adults Experiencing Poor\n Mental Health 14 days or more")

```

The model fit did not change. The interaction between SNAP utilization and maximum educational attainment is only significant for GED. The coefficient is positive as is the coefficient for pct_SNAP indicating the relationship between SNAP utilization and poor mental health days is stronger in census tracts that have GED as the maximum educational attainment. 


## pct_SNAP and pct_UnitsWChildren
```{r}
...
The model fit did not change. The interaction between SNAP utilization and units with children is positive and significant. This indicates census tracts with higher utilization of SNAP and higher percentages of households with children experience higher amounts of poor mental health days. 


## median income and access to food within 1 mile urban and 10 mile rural
```{r}
coeff_names_interaction <- c("Constant" = "(Intercept)",
                 "SNAP Utilization" = "pct_SNAP",
                 "Poverty Rate" = "PovertyRate",
...
The model fit does not change with this interaction, but the interaction between food access and logged median income is significant with a p-value of 0.01. There is a slight difference in the relationship between logged median income and poor mental health when interacting with food access; for areas with food access, median income has less of an effect on poor mental health days.


## pct_SNAP and access to food within 1 mile urban and 10 mile rural
``` {r}
coeff_names_interaction <- c("Constant" = "(Intercept)",
                 "SNAP Utilization" = "pct_SNAP",
                 "Poverty Rate" = "PovertyRate",
...
Including an interaction between percent SNAP utilization and food access by distance did not change the model fit. This interaction revealed that poor mental health rates increased at a greater rate as SNAP utilization increased for census tracts within the definition of food access (10 miles in a rural area and 1 mile in an urban area), compared to census tracts out of the food access area. This is a surprising result as we had expected to see increased food access by distance lessen the effects on mental health.
...
Adding the  interaction coefficients described above to the regression analysis did not change the model fit compared to the model with the log of median income. The model of logged median income and the interaction coefficient for percent SNAP usage and food access is our preferred model because it is among the models with the best fit (R-squared value of 0.85) and it leads to more questions for further research. What other factors are leading to the difference in mental health as correlated with SNAP usage when comparing areas of high food access to areas of low food access?
