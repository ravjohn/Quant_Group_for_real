...
```{r}
full_model <- lm(pct_PMH ~ pct_SNAP + PovertyRate + MedIncomeE + MedAge + pct_UnitsWChildren + pct_RWhite + pct_RBlack + pct_RAI + pct_RAsian + pct_RNH + pct_ROther + pct_RTwoPlus+ LA1and10 + MaxEd, data)

summary(full_model)
```
## Combine RTwoPlus and ROther
```{r}
data <- data %>%
  mutate(pct_RTPO = pct_RTwoPlus + pct_ROther)
```

## Run regression with pct_RTPO variable
```{r}
full_model2 <- lm(pct_PMH ~ pct_SNAP + PovertyRate + MedIncomeE + MedAge + pct_UnitsWChildren + pct_RWhite + pct_RBlack + pct_RAI + pct_RAsian + pct_RNH + pct_RTPO + LA1and10 + MaxEd, data)

summary(full_model2)
```
Start with summary of variables that affect correlation. Then mention significance. Then detail variables.

The model's R squared value is 0.843, so this combination of independent variables explains 84% of the variation in percent of population experiencing poor mental health for more than 14 days.

Positive Correlation:
As percent SNAP usage, Poverty Rate, etc. increase, pct_PMH increases.

Holding all else constant, with a 1% increase in pct_SNAP, pct_PMH increases by 0.0177%.
Holding all else constant, with a 1% increase in Poverty Rate, pct_PMH increases by 0.001386%.

Negative Correlation
Holding all else constant, a $10,000 increase in median income is associated with a decrease in pct_PMH by 0.006253%.


(? Intercept describing two categorical variables?)
1 not defined beceause of singularities - Two or More Races is almost perfectly correlated with another data point. Try to combine ROther and RTwoPlus
